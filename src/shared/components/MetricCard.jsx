...
const MetricCard = ({ 
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-[#111827]/40 backdrop-blur-md border rounded-3xl p-6 overflow-hidden transition-all shadow-lg ${statusColors[status]} ${className}`}
    >
      {/* Glow highlight */}
      <div className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-[80px] pointer-events-none transition-colors duration-500 -mr-8 -mt-8 ${glowColors[status]}`} />
      ...
      <div className={`text-3xl font-extrabold tracking-tight ${textColors[status]} mb-1.5`}>
        {value}
      </div>
